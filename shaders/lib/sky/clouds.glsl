...
float SampleCloudDensity(const in vec3 worldPos) {
    ...
    float cloud_density = mix(0.05, 6.4, ap.world.rainStrength);
    float cloud_threshold = mix(0.28, 0.08, ap.world.rainStrength);
...
float SampleCloudDensity2(const in vec3 worldPos) {
    ...
    float cloud_density = mix(0.1, 40.0, ap.world.rainStrength);
