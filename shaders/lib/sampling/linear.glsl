...
float TextureLodLinear(const in sampler2D samplerName, const in vec2 texcoord, const in vec2 texSize, const in float lod, const in int comp) {
    vec2 uv[4];
    vec2 f = GetLinearCoords(texcoord, texSize, uv);
...
vec3 TextureLodLinearRGB(const in sampler2D samplerName, const in vec2 uv[4], const in float lod, const in vec2 f) {
    vec3 samples[4];
...
vec3 TextureLodLinearRGB(const in sampler2D samplerName, const in vec2 texcoord, const in vec2 texSize, const in float lod) {
    vec2 uv[4];
    vec2 f = GetLinearCoords(texcoord, texSize, uv);
